...
--
--
-- select y.st,round(((y.res+y.gpa)/2),1) as result,
--  y.fio, y.group_id
--  from (select x.st as st,
--   round(avg(x.avg_user),1) as res,
--   x.fio as fio, x.gpa as gpa,
--   x.group_id as group_id from
--   (select student_id as st,student_fio
--   as fio,student_gpa as gpa, group_id as group_id,
--   round(((round(((g1+g2+g3_1+g3_2+g3_3)/5),1)
--   +round(((g4_1+g4_2+g4_3+g4_4+g4_5+g4_6)/6),1))/2),1)
--   as avg_user from lists join students
--    n lists.student_id=students.id )
--    x group by st,fio, gpa, group_id)y
select * from docs;
